<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    
    <link rel="stylesheet" href="CSS/iloveyousabado.css">

    <script type="module" src="https://unpkg.com/ionicons@7.1.0/dist/ionicons/ionicons.esm.js"></script>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Roboto+Condensed:ital,wght@0,100..900;1,100..900&display=swap" rel="stylesheet">
    <link rel="icon" href="icon.ico" type="image/x-icon">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">
    <link rel="icon" href="Website/icon.ico" type="image/x-icon">
    <script src="https://unpkg.com/sweetalert/dist/sweetalert.min.js"></script>
    

    <meta name="viewport" content="width=device-width, initial-scale=2.0">
    
    <title>Travel go ph</title>

</head>

<body>
    
   <form action="index.php" method="post">
    <section class="banner">
    <header>
    
    <nav class="navigation">
           
        <div class="header">

            <div class="nav2">

            <div class="travelgoPHmember">
                
            </div>

             <a  href="calendar.php" class="bookings_ko">My Bookings</a>


                <h1 class="travel">Travel Go PH</h1>

                
                <div class="wrapper">
                    <input type="search" id="search" class="searchbar" placeholder="Search..." list="colors" autocomplete="off">
                        <div class="my-box" id="my-box">
                           
                            

                        
                </div>
                
                


              
                    
                </div>

            </div>
            
            
            
          
            <ul class="myul">
           
                
                <li class="li"><a href="#" class="a">Home</a> </li>
          
                <li class="li"><a href="#"class="a">Hotels</a></li>
            
                <li class="lis"><a href="#"class="a">Attractions</a></li>

                <li class="lis"><a href="#"class="a">Promo</a></li>

                <li class="lis"><a href="#"class="a">Flights</a></li>

               

                

                 
            
                <li class="list"><button class="button" name="button">Signout</button></li><?php
                
                
                if (isset($_POST["button"])) {
                    header("Location: login_page.php");
                }
                
                
                ?>



             
            </ul>


        
        <p class="travel-2">Welcome to Manila!</p>

        

        <div class="container_for-inputs">
           
            <input type="date" name="" id="">

            
        </div>
        </div>

    </nav>
   
</header>



    

</section>


<section class="sec2">
    
   
    <!-- ==========================================================================number 1 container ===============================================================================-->
    <div class="container">


  
        <h1 class="for">Popular Attractions</h1>

        <?php
    $conn = new mysqli("localhost:3307", "root", "admin", "sample");

    if ($conn->connect_error) {
        die("Connection failed: " . $conn->connect_error);
    }

    // SQL query to retrieve image data
    $sql = "SELECT place, image FROM for_creating_a_place"; // Adjust the query according to your database schema
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        // Output data of each row
        while($row = $result->fetch_assoc()) {
         // Display the image
            $imageData = base64_encode($row['image']);
            $textData = $row['place'];
            echo '<div class="inner-box">
                  <div class="box"><img src="img/' . $row["image"] . '" alt="Image" /></div>
                  <h2>'.$textData.'</h2>
              </div>';


             
        }
        } else {
        echo "0 results";
        }
       

        if (isset($_POST["inner-box"])) {
            echo '<script>sweetAlert("dont have any account?", "You dont have any account? Sign-in", "warning");</script>';
        }
        

           
          
?>
        
        
        
 </div>


     <!--====================================================================================== NUMBER 2 CONTAINER================================================================================ -->
     <div class="container2">
    <h1 class="promos">Promos</h1>

    <div class="box3">
        <img src="Book your Hotels Stay now with a Discount of up to 30% with Savings24x7.jpg">
    </div>
    
    <div class="box3">
        <img src="AirAsia B1G Sale drums up 2023 travel with Php 1 fare Promo.jpg">
    </div>
</div>




<!-----------------------------------------------------------------------------------------------CONTAINER 3--------------------------------------------------------------------------------------------->
<div class="container3">
<h1 class="featured_properties">
    Hotel Recommendations
</h1>
        <ul>
            <li class="listt" onclick=""><span class="js-tab-item">Manila</span></li>
            <li class="listt" onclick=""><span class="js-tab-item">Cebu</span></li>
            <li class="listt" onclick=""><span>Bohol</span></li>
            <li class="lis"   onclick=""><span>Pangasinan</span></li>
            <li class="lis"   onclick=""><span>Davao</span></li>
            <li class="lis"   onclick=""><span>Albay</span></li>
            
        </ul>



        <div class="hotels" id="hotels">
            <div class="in">
            <img src="https://ak-d.tripcdn.com/images/02009120006rf0f3o7168_R_960_660_R5_D.webp" alt="" srcset="">
            </div>
            <h1>Hotel Savoy Manila</h1>
            <i class="fas fa-star"></i>
            <i class="fas fa-star"></i>
            <i class="fas fa-star"></i>
            <i class="fas fa-star"></i>
            <i class="fas fa-star"></i>

            <div class="reviews">
               <p>5.0/5</p> 
               
            </div>

            <h1 class="h2">Starts from PHP 4,500</h1>
        </div>
        
        <div class="hotels" id="hotels"  onclick="">
        <div class="in">
            <img src="https://ak-d.tripcdn.com/images/0223h12000arcfzy51178_R_960_660_R5_D.webp" alt="" srcset="">
            </div>
            <h1>Eurotel Pedro Gil</h1>
            <i class="fas fa-star" id="con"></i>
            <i class="fas fa-star" id="con"></i>
            <i class="fas fa-star" id="con"></i>
            <div class="reviews">
               <p>5.0/5</p> 
               
            </div>

            <h1 class="h3">Starts from PHP 7,579</h1>
            
        </div>
        
        <div class="hotels" id="hotels" onclick="">
        <div class="in">
            <img src="https://ak-d.tripcdn.com/images/0220d12000aqhn3ak958F_R_960_660_R5_D.webp" alt="" srcset="">
            </div>
            <h1>The Alpha Suites</h1>
            <i class="fas fa-star" id="cons"></i>
            <i class="fas fa-star" id="cons"></i>
            <i class="fas fa-star" id="cons"></i>
            <i class="fas fa-star" id="cons"></i>
            <i class="fas fa-star" id="cons"></i>

            <div class="reviews">
               <p>5.0/5</p> 
               
            </div>

            <h1 class="h2">Starts from PHP 10,531</h1>
        </div>
        
        <div class="hotels" id="hotels" onclick="">
        <div class="in">
            <img src="https://ak-d.tripcdn.com/images/0584n12000cs7f2jl27C4_R_960_660_R5_D.webp" alt="" srcset="">
            </div>
            <h1>Swiss-Belhotel Blulane Manila Philippines</h1>
            
            <i class="fas fa-star" id="icon"></i>
            <i class="fas fa-star" id="icon"></i>
            <i class="fas fa-star" id="icon"></i>
            <i class="fas fa-star" id="icon"></i>
            <i class="fas fa-star" id="icon"></i>


            <div class="reviews">
               <p>5.0/5</p> 
               
            </div>


            <h1 class="h1">Starts from PHP 4,500</h1>
            



        </div>
        
       
        
       


        </div>

       <div class="contacts">

       <h3 class="h1">Contacts</h3>
      




       <h3 class="h2">About</h3>

       

       
       <h3 class="h4">Payment Methods</h3>
       <li class="cl"><a href="#">Customer Support</a></li>
       <li class="cl"><a href="#">Service Guarantee</a></li>
       <li class="cl"><a href="#">More Service Info</a></li>
       <li class="cl"><a href="#">Website Feedback</a></li>
       <li class="cl"><a href="#">Developers</a></li>

       

       <li class="cll"><a href="">About Travel Go Philippines</a></li>
       <li class="cll"><a href="">Travel go promos</a></li>
       <li class="cll"><a href="">Travel Go Group</a></li>
       <li class="cll"><a href="">Travel Go Quality</a></li>
       <li class="cll"><a href="">Terms and Conditions</a></li>
       <li class="cll"><a href="">Privacy Statement</a></li>

       <img src="https://ak-d.tripcdn.com/images/05E2712000cjsr5ul9716.png" alt="" srcset="">
       <img src="https://ak-d.tripcdn.com/images/05E0f12000cjsr2f9AAAB.png" alt="" srcset="">
       <img src="https://ak-d.tripcdn.com/images/05E4f12000cjsqzn4B29D.png" alt="">
       <img src="https://ak-d.tripcdn.com/images/05E3e12000cjsr47e852C.png" alt="">
       <img class="" src="data:image/png;base64,"alt="">
       <img class="" src="data:image/png;base64," alt="">

       <h3>follow </h3>
       </div>
        
    
    
    
    
       <footer>
        <hr>
       <p>Copyright © 2024 Travel Go Philippines. All rights reserved.</p>
   </footer> 
        
       
    
</section>

 

<script src="script.js"></script>


    
</body>
</html>





